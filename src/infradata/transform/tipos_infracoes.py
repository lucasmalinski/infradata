...
def transform_all (infr_dict: dict) -> pd.DataFrame:
    infr_dict = {year: _transform(df, year) for year,df in infr_dict.items()}

    df_concat = pd.concat(infr_dict.values(), axis=0)
    return df_concat
